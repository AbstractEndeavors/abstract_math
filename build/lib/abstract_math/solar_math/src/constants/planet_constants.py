...
def hill_radius(
    ...
    input_dist_unit = "meters"  # body data is in meters internally
...
def gravity_reach(
    ...
    normalized_output_dist_unit = normalize_distance_unit(output_dist_unit)
    input_dist_unit = "meters"  # mu is m^3/s^2, threshold is m/s^2 → r is in meters
...
def earth_radius(
    ...
    normalized_units = normalize_distance_unit(input_dist_unit)
    radius = planet_radius(planet='earth', input_dist_unit=input_dist_unit)
    if not to_dict:
        return radius
    return {
            "input": {
                "input_dist_unit": normalized_units,
            },
            "output": {
                "radius": radius,
                "output_dist_unit": normalized_units,
            }
        }
